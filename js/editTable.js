...
let table1 = document.getElementsByClassName('project-table');
...
for(let j=0; j<table1.length; j++){
    let cells = table1[j].getElementsByTagName('td');
            ...
            input.onblur = function(){
                ...
                if(old_text != new_text){
                    ...
                    let $row = $(this).closest("tr"),         // Finds the closest row <tr>
                        ...
                        $tdLast = $row.find("td:last-child");
                    
                    console.log("Header Under Clicked: "+$(this).closest('table').find('th').eq($cell.index()).text());
                    
                    $.each($tds, function(){                // Visits every single <td> element
                        console.log("AIPRefCode: "+$(this).text()); // returns the AIP Reference Code
                        console.log("ID: "+$tdLast.text()); // ID
                    });
